...
describe('Pair', () => {
  ...
  describe('constructor', () => {
    it('cannot be used for tokens on different chains', () => {
      expect(() => new Pair(new TokenAmount(FISH, '100'), new TokenAmount(WAVAX[ChainId.AVALANCHE], '100'), ChainId.FUJI)).toThrow(
        'CHAIN_IDS'
      )
    })
  })
  ...
  describe('#getAddress', () => {
    it('returns the correct address', () => {
      expect(Pair.getAddress(FISH, MURLOC, ChainId.FUJI)).toEqual('0x9c1111fed56e51790a74Ea02B054e509826840E7')
    })
  })
  ...
  describe('#token1', () => {
    it('always is the token that sorts after', () => {
      expect(new Pair(new TokenAmount(MURLOC, '100'), new TokenAmount(FISH, '100'), ChainId.FUJI).token1).toEqual(MURLOC)
      expect(new Pair(new TokenAmount(FISH, '100'), new TokenAmount(MURLOC, '100'), ChainId.FUJI).token1).toEqual(MURLOC)
    })
  })
  describe('#reserve0', () => {
  ...
  describe('#reserveOf', () => {
    it('returns reserves of the given token', () => {
      expect(new Pair(new TokenAmount(MURLOC, '100'), new TokenAmount(FISH, '101'), ChainId.FUJI).reserveOf(MURLOC)).toEqual(
        new TokenAmount(MURLOC, '100')
      )
      expect(new Pair(new TokenAmount(FISH, '101'), new TokenAmount(MURLOC, '100'), ChainId.FUJI).reserveOf(MURLOC)).toEqual(
        new TokenAmount(MURLOC, '100')
      )
    })
    ...
    it('throws if not in the pair', () => {
      expect(() =>
        new Pair(new TokenAmount(FISH, '101'), new TokenAmount(MURLOC, '100'), ChainId.FUJI).reserveOf(WAVAX[ChainId.FUJI])
      ).toThrow('TOKEN')
    })
  })
  ...
  describe('#chainId', () => {
    it('returns the token0 chainId', () => {
      expect(new Pair(new TokenAmount(MURLOC, '100'), new TokenAmount(FISH, '100'), ChainId.FUJI).chainId).toEqual(ChainId.FUJI)
      expect(new Pair(new TokenAmount(FISH, '100'), new TokenAmount(MURLOC, '100'), ChainId.FUJI).chainId).toEqual(ChainId.FUJI)
    })
  })
  describe('#involvesToken', () => {
    expect(new Pair(new TokenAmount(MURLOC, '100'), new TokenAmount(FISH, '100'), ChainId.FUJI).involvesToken(MURLOC)).toEqual(true)
    expect(new Pair(new TokenAmount(MURLOC, '100'), new TokenAmount(FISH, '100'), ChainId.FUJI).involvesToken(FISH)).toEqual(true)
    expect(
      new Pair(new TokenAmount(MURLOC, '100'), new TokenAmount(FISH, '100'), ChainId.FUJI).involvesToken(WAVAX[ChainId.FUJI])
    ).toEqual(false)
  })
})
